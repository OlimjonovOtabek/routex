...
import {
 DriverAnimation, // Тип для ID анимации
 angleFromCoordinate,
 animate,
 splitLineString,
} from "../common"; // <-- Уточните путь к common.ts
...
import {
 ...
 PASSED_ROUTE_STYLE,
 ROUTE_STYLE,
} from "../variables"; // <-- Уточните путь к variables.ts

// --- Глобальные объявления для Яндекс.Карт API (инжектируются скриптом в index.html) ---
// Это крайне важно для TypeScript, чтобы он "знал" о глобальном объекте `ymaps3`.
declare global {
 interface Window {
  ymaps3: {
   ready: Promise<void>;
   YMap: typeof import("@yandex/ymaps3-types").YMap;
   YMapDefaultSchemeLayer: typeof import("@yandex/ymaps3-types").YMapDefaultSchemeLayer;
   YMapDefaultFeaturesLayer: typeof import("@yandex/ymaps3-types").YMapDefaultFeaturesLayer; // Добавлено
   YMapMarker: typeof import("@yandex/ymaps3-types").YMapMarker;
   // YMapLine: typeof import("@yandex/ymaps3-types").;
   YMapFeature: typeof import("@yandex/ymaps3-types").YMapFeature; // Добавлено для работы с линиями
   //  YMapControls: typeof import("ymaps3").YMapControls; // Removed
   // YMapControl: typeof import("ymaps3").YMapControl; // Removed
   // Если используется @yandex/ymaps3-default-ui-theme
   import: (packageName: string) => Promise<{
    YMapDefaultMarker: typeof import("@yandex/ymaps3-default-ui-theme").YMapDefaultMarker;
   }>;
  };
 }
}
...
interface Assignment {
 ...
 brigadeLocation: {
  latitude: number; // Текущая локация бригады
  longitude: number;
 };
 destinationFullAddressLine: string;
 name: string; // Имя бригады (например, "Бригада А")
 nameRu: string; // Название бригады на русском
 nameUz: string | null;
 nameKa: string | null;
}

// Новый интерфейс для объединенных данных отслеживания
interface BrigadeTrackingData {
 brigadeInfo: {
  name: string; // Например, 'Бригада 103'
  carNumber: string | null; // Номер автомобиля
 };
 currentLocation: LngLat; // [longitude, latitude]
 routeCoordinates: LngLat[]; // Массив точек маршрута от бригады до пациента
 status: "enRoute" | "arrived" | "cancelled" | "pending"; // Добавлен 'pending' для начального состояния
 patientLocation: LngLat; // Локация пациента
 patientAddress: string; // Полный адрес пациента
}

// --- Глобальные переменные состояния приложения ---
let mapInstance: InstanceType<typeof window.ymaps3.YMap> | null = null;
let brigadeMarker: InstanceType<typeof window.ymaps3.YMapMarker> | null = null;
let routeLinePassed: InstanceType<typeof window.ymaps3.YMapFeature> | null =
 null; // Пройденный маршрут
let routeLineRemaining: InstanceType<typeof window.ymaps3.YMapFeature> | null =
 null; // Оставшийся маршрут

let currentBrigadeLocation: LngLat | null = null; // Текущая локация бригады
let currentRoute: LngLat[] | null = null; // Текущий маршрут
let animationFrameId: DriverAnimation | null = null; // ID текущей анимации

let pollingInterval: number | null = null; // ID интервала для polling'а
...
const activeTrackingPanel = document.getElementById(
 "active-tracking-panel"
) as HTMLElement;
const finishedCallAlert = document.getElementById(
 "finished-call-alert"
) as HTMLElement;
const brigadeNameSpan = document.getElementById(
 "brigade-name"
) as HTMLSpanElement;
const carNumberSpan = document.getElementById("car-number") as HTMLSpanElement;
const patientAddressSpan = document.getElementById(
 "patient-address"
) as HTMLSpanElement;
 ...
 */
function getCallIdFromUrl(): string | null {
...
async function getToken(
 assignmentId: string = "6fed6b78-47c6-4de9-b033-ad4b5d3da7a3", // Заменить на реальные данные
 createdAt: string = "2025-06-19T12:26:59.155017+05:00" // Заменить на реальные данные
): Promise<string> {
 try {
 ...
 */
async function fetchBrigadeTrackingData(
 callId: string,
 token: string
): Promise<BrigadeTrackingData | null> {
 try {
  ...
  const response = await fetch(
   `https://103.init.uz/brigade-tracking-service/api/brigade-tracking/assignments/oJTfJVA1qQl5YFKEWmgaeAHHKTXFmQVkFwHap7iuP-I.` // Это кажется эндпоинтом для 'assignments'
  ...
  const assignmentData: Assignment = await response.json(); // Получаем данные Assignment
  console.log("Получены Assignment:", assignmentData);

  // Дополнительный запрос для локации бригады, если он отдельный
  const brigadeLocationResponse = await fetch(
   `https://103.init.uz/brigade-tracking-service/api/brigade-tracking/brigade-location/${assignmentData.brigadeId}?token=${token}` // Используем brigadeId
  );
  if (!brigadeLocationResponse.ok) {
   throw new Error(
    `Ошибка получения локации бригады: ${brigadeLocationResponse.statusText}`
   );
  }
  const brigadeLocationData: { latitude: number; longitude: number } =
   await brigadeLocationResponse.json(); // Use anonymous type for immediate destructuring if needed
  ...
  // Имитируем статус
  let currentStatus: BrigadeTrackingData["status"] = "enRoute";
  // Для демо: если бригада близко к пациенту, меняем статус на "arrived"
  const distanceToPatient = length(
   lineString([
    [brigadeLocationData.longitude, brigadeLocationData.latitude], // Use actual brigadeLocationData
    [
     assignmentData.destinationLocation.longitude,
     assignmentData.destinationLocation.latitude,
    ],
  ...
  return {
   ...
   ], // Use actual brigadeLocationData
   routeCoordinates: mockRoute, // Здесь должны быть реальные координаты маршрута от бэкенда
 ...
 */
function animateBrigadeToNewLocation(
 fromCoords: LngLat,
 toCoords: LngLat,
 newRoute: LngLat[]
) {
 ...
 const segment = lineString([fromCoords as any, toCoords]);
 ...
 animationFrameId = animate((progress) => {
  ...
  if (
   prevAnimatedCoords &&
   !booleanEqual(
    point(prevAnimatedCoords as any),
    point(currentCoordinates as any)
   )
  ) {
  ...
  const [passedLineString, remainingLineString] = splitLineString(
   lineString(newRoute as any) as any, // Используем весь новый маршрут
   currentCoordinates
  );
 ...
 */
function updateUI(data: BrigadeTrackingData) {
 ...
 if (
  currentBrigadeLocation &&
  !booleanEqual(
   point(currentBrigadeLocation as any),
   point(data.currentLocation as any)
  )
 ) {
  animateBrigadeToNewLocation(
   currentBrigadeLocation,
  ...
  mapInstance.setLocation(
   [data.patientLocation, data.currentLocation].concat(
    data.routeCoordinates || []
   ) as any
  );
 }
 ...
 */
function handleCallStatus(status: BrigadeTrackingData["status"]) {
 if (status === "arrived" || status === "cancelled") {
  ...
  if (animationFrameId !== null) {
   cancelAnimationFrame(animationFrameId.getAnimationId() as number);
...
async function main() {
 ...
 mapInstance = new ymaps3.YMap(
  ...
  {
   location: {
    center: [69.2401, 41.2995], // Начальный центр карты (например, Ташкент)
    zoom: 10,
   },
 ...
 brigadeMarker = new ymaps3.YMapMarker(
  {
   coordinates: [0, 0], // Начальные координаты будут обновлены
 ...
 let TOKEN: string; // Токен, если используется
 try {
  // ВНИМАНИЕ: Замените ID на реальные, если токен зависит от assignmentId
  TOKEN = await getToken(
   "6fed6b78-47c6-4de9-b033-ad4b5d3da7a3",
   "2025-06-19T12:26:59.155017+05:00"
  );
 } catch (error) {
 ...
 if (initialData.status === "enRoute") {
  ...
  // Устанавливаем карту так, чтобы были видны бригада и пациент
  mapInstance.setLocation(
   [initialData.patientLocation, initialData.currentLocation].concat(
