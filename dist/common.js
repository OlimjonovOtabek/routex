...
export async function fetchRoute(startCoordinates, endCoordinates) {
...
export async function getToken(assignmentId = "6fed6b78-47c6-4de9-b033-ad4b5d3da7a3", createdAt = "2025-06-19T12:26:59.155017+05:00") {
    return fetch(`https://103.init.uz/brigade-tracking-service/api/brigade-tracking/get-token?assignmentId=${assignmentId}&createdAt=${createdAt}`);
}
export async function getAssignments() {
    return fetch(`https://103.init.uz/brigade-tracking-service/api/brigade-tracking/assignments/oJTfJVA1qQl5YFKEWmgaeAHHKTXFmQVkFwHap7iuP-I.`);
}
export async function getBrigadeLocation(id = "09fc261e-c281-49be-8d7d-af8f2169f5c5", token = "hbCt-UsCRGgLF_I_ohwAIhWUsJ4XyE37KJ0mRDRy7FY.") {
    return fetch(`https://103.init.uz/brigade-tracking-service/api/brigade-tracking/brigade-location/${id}?token=oJTfJVA1qQl5YFKEWmgaeAHHKTXFmQVkFwHap7iuP-I.`);
}
export function angleFromCoordinate(lngLat1, lngLat2) {
    const toRadians = (degrees) => degrees * (Math.PI / 180);
    const toDegrees = (radians) => radians * (180 / Math.PI);
    const dLon = toRadians(lngLat2[0] - lngLat1[0]);
    const y = Math.sin(dLon) * Math.cos(toRadians(lngLat2[1]));
    const x = Math.cos(toRadians(lngLat1[1])) * Math.sin(toRadians(lngLat2[1])) -
...
export function splitLineString(route, coordinates) {
    if (!route || !coordinates) {
        return [];
    }
    route.geometry = cleanCoords(route.geometry);
    const firstPart = lineSlice(coordinates, route.geometry.coordinates[route.geometry.coordinates.length - 1], route.geometry);
    const secondPart = lineSlice(route.geometry.coordinates[0], coordinates, route.geometry);
    return [
        firstPart.geometry,
        secondPart.geometry,
    ];
}
export function getCenterCoordinate(start, end) {
